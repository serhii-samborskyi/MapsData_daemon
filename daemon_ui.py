...
class DaemonUI(QtWidgets.QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.pipeline_worker_pool_size = QtWidgets.QSpinBox()
        self.pipeline_worker_pool_size.setRange(1, 10)
        ...
        self.pipeline_fast_email_policy = QtWidgets.QComboBox()
        self.pipeline_fast_email_policy.addItem("Business domain only (strict)", "business_only")
        self.pipeline_fast_email_policy.addItem("Business or public inboxes", "business_or_public")
        self.pipeline_fast_email_policy.addItem("Any valid email (legacy)", "any_valid")
        ...
        self.pipeline_fallback_email_policy = QtWidgets.QComboBox()
        self.pipeline_fallback_email_policy.addItem("Business domain only (strict)", "business_only")
        self.pipeline_fallback_email_policy.addItem("Business or public inboxes", "business_or_public")
        self.pipeline_fallback_email_policy.addItem("Any valid email (legacy)", "any_valid")
        ...
        pipeline_form.addRow("Pipeline worker pool", self.pipeline_worker_pool_size)
        ...
        pipeline_form.addRow("Fast stage email policy", self.pipeline_fast_email_policy)
        ...
        pipeline_form.addRow("Fallback stage email policy", self.pipeline_fallback_email_policy)
        ...
        self.email_show_browser = QtWidgets.QCheckBox("Show browser window while scraping emails")
        ...
        email_form.addRow("", self.email_show_browser)
    ...
    def _load_config_to_form(self) -> None:
        ...
        self.pipeline_worker_pool_size.setValue(int(pipeline_cfg.get("worker_pool_size", 10)))
        ...
        fast_policy = str(pipeline_cfg.get("fast_email_policy", "business_only")).lower().strip()
        fast_policy_index = self.pipeline_fast_email_policy.findData(fast_policy)
        if fast_policy_index == -1:
            fast_policy_index = 0
        ...
        fallback_policy = str(pipeline_cfg.get("fallback_email_policy", "business_or_public")).lower().strip()
        fallback_policy_index = self.pipeline_fallback_email_policy.findData(fallback_policy)
        if fallback_policy_index == -1:
            fallback_policy_index = 1
        self.pipeline_fallback_email_policy.setCurrentIndex(fallback_policy_index)
        ...
        self.email_show_browser.setChecked(bool(email_cfg.get("show_browser", False)))
    ...
    def _collect_form_config(self) -> Dict[str, Any]:
        ...
        pipeline_cfg["worker_pool_size"] = int(self.pipeline_worker_pool_size.value())
        ...
        pipeline_cfg["fallback_email_policy"] = str(self.pipeline_fallback_email_policy.currentData() or "business_or_public")
        ...
        cfg["email"]["show_browser"] = bool(self.email_show_browser.isChecked())
